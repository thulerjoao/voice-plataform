...
import { CreateRoomScreen } from "../home/components/create-room";
import { JoinRoomScreen } from "../home/components/join-room";
...
function RoomActions({
  ...
  return (
    <Actions $compact={compact} style={compact ? undefined : { justifyContent: "center" }}>
...
export function HomeScreen({
  ...
  const presenceStatusRef = useRef(status);
        ...
        {view.type === "home" ? (
          <HomePane>
            ...
            {rooms.length === 0 ? (
              ...
              <ServerList>
                {rooms.map((room) => {
                  const seated = occupancyByRoom[room.roomId]?.length ?? 0;
                  const live = call?.roomId === room.roomId;
                  const mark =
                    room.role === "owner"
                      ? "owner"
                  ...
                  return (
                    ...
                    >
                      ...
                      <ServerCopy>
                        <ServerName>{room.name}</ServerName>
                        <ServerCode>
                          {normalizeRoomCode(room.code) || room.code}
                        </ServerCode>
                      </ServerCopy>
                      <Occupancy>
                        <OccupancyIcon />
