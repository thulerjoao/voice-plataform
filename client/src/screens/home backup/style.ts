...
export const SidebarAddMark = styled.span`
  display: grid;
  place-items: center;
  width: 1.05rem;
  height: 1.05rem;
...
export const HomePane = styled.div`
  width: 100%;
  max-width: 72rem;
  margin: 0 auto;
  flex: 1;
...
export const Actions = styled.div<{ $compact?: boolean }>`
  display: flex;
  flex-wrap: wrap;
  gap: ${(p) => (p.$compact ? "0.75rem" : "1rem")};
  flex-shrink: 0;

  & > button {
    min-width: ${(p) => (p.$compact ? "auto" : "13.5rem")};
  }
`;
